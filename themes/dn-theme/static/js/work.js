...
function resizeMasonryItem(item) {
    ...
    gridImagesAsContent = item.querySelector('.masonry-item > *');
...
/*
    ...
    * S = H1 / T
    */
var rowSpan = Math.ceil((item.querySelector('.masonry - item > *').getBoundingClientRect().height + rowGap) / (rowHeight + rowGap));

/* Set the spanning as calculated above (S) */
item.style.gridRowEnd = 'span ' + rowSpan;
                if (gridImagesAsContent) {
    item.querySelector('.masonry-item > *').style.height = item.getBoundingClientRect().height + "px";
}
}
}
